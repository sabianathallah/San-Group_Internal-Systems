...
// ── Task row ───────────────────────────────────────────────
function TaskRow({ task, selected, onSelect, onToggle, onDelete, showUser }: {
  task: Task; selected: boolean; onSelect: () => void;
  onToggle: () => void; onDelete: () => void; showUser?: boolean;
}) {
  ...
  return (
    <div
      onClick={onSelect}
      className={cn(
        'group flex items-center gap-3 px-4 py-2.5 border-b border-gray-50 cursor-pointer transition-colors',
        'border-l-2',
        selected ? 'bg-navy/5 border-l-navy' : `hover:bg-gray-50/70 ${PRIORITY_CONFIG[task.priority].border}`,
      )}
    >
      <button
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 min-w-0">
          <span className={cn('text-sm truncate', done && 'line-through text-gray-400')}>
            {task.title}
          </span>
          {task.isPrivate && <Lock size={11} className="text-gray-300 flex-shrink-0" />}
          {task.assignmentStatus && <AssignBadge status={task.assignmentStatus} />}
        </div>
        {showUser && task.creator && (
          <p className="text-[11px] text-gray-400 truncate">{task.creator.fullName}</p>
        )}
      </div>
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        ...
        {due.text && (
          <span className={cn('text-[11px]', due.overdue ? 'text-red-500 font-medium' : 'text-gray-400')}>
            {due.text}
          </span>
        )}
        {task.assignee && (
